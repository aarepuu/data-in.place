...
!function (exports) {
    ...
    raw.parser = function (delimiter) {
        ...
        function sniff(objs) {
            var keys = {};
            d3.keys(objs[0]).forEach(function (d) {
                keys[d] = [];
            });
        ...
        function detectGeometry(string) {
...
//  new RegExp('^([A-z]{3,}(\\,)?\\s+)?(0?[1-9]|[12][0-9]|3[01])([A-z]{2})?[\\-\\_\\.\\/\\s]*(0?[1-9]|1[012])[\\-\\_\\.\\/\\s]*([0-9]{2,4})' + timeFormat + '$'),
    ...
    raw.lnglatFormats = [
        new RegExp(/^(\+|-)?(?:180(?:(?:\.0{1,6})?)|(?:[0-9]|[1-9][0-9]|1[0-7][0-9])(?:(?:\.[0-9]{1,6})?))$/g), //latitude
        new RegExp(/^(\+|-)?(?:180(?:(?:\.0{1,6})?)|(?:[0-9]|[1-9][0-9]|1[0-7][0-9])(?:(?:\.[0-9]{1,6})?))$/g) //longitude
    ]
    ...
    raw.isLnglat = function (value) {
        var isLnglat = false;
        for (var format in raw.lnglatFormats) {
            if (value.trim().match(raw.lnglatFormats[format])) {
                isLnglat = true;
                break;
            }
